...
st.markdown("""
    <style>
        ...
        /* Specific Button Colors inside Nav Capsule */
        button[key="tab_btn_form"] {
        ...
        button[key="tab_btn_hist"] {
            background-color: #2B71F2 !important;
            color: #FFFFFF !important;
            border: none !important;
            border-radius: 20px !important;
            height: 36px !important;
        ...
        /* High-Contrast Corporate Inputs matching image_22d11a.png */
...
# Top utilities above the capsule
...
st.markdown('<div class="nav-capsule-bg">', unsafe_allow_html=True)
if user['role'] == "Admin":
    n_col = st.columns([3.4, 1.3, 1.3, 1.4, 1.3, 1.3])
    n_col[0].markdown(f'<div class="nav-user-label-styled"><span style="color:#FF6B00;">🛡️</span> {user["name"]} ({user["role"]})</div>', unsafe_allow_html=True)
    if n_col[1].button("📄 OT Form", key="tab_btn_form", use_container_width=True):
        st.session_state.current_view = "portal"
        st.rerun()
    if n_col[2].button("🕒 History", key="tab_btn_hist", use_container_width=True):
        st.session_state.current_view = "history"
        st.rerun()
    if n_col[3].button("📊 Dashboard", key="tab_btn_dash", use_container_width=True):
        st.session_state.current_view = "dashboard"
        st.rerun()
    if n_col[4].button("📈 Reports", key="tab_btn_rep", use_container_width=True):
        st.session_state.current_view = "reports"
        st.rerun()
    if n_col[5].button("👥 Admin", key="tab_btn_adm", use_container_width=True):
    ...
    if n_col[1].button("📄 OT Form", key="tab_btn_form", use_container_width=True):
        st.session_state.current_view = "portal"
        st.rerun()
    if n_col[2].button("📊 Dashboard", key="tab_btn_dash", use_container_width=True):
        st.session_state.current_view = "dashboard"
        st.rerun()
    if n_col[3].button("📈 Reports", key="tab_btn_rep", use_container_width=True):
        st.session_state.current_view = "reports"
        st.rerun()
else:
...
if st.session_state.current_view == "portal":
    with st.container(border=True):
        st.markdown("""
            <div style="border-top: 5px solid #FF6B00; margin: -16px -16px 20px -16px;"></div>
            <div style="text-align: center; margin-bottom: 24px;">
                <div style="font-size: 26px; font-weight: 800; color: #0E2B5C;">EXCIT<span style="color:#FF6B00;">EL</span></div>
...
# ==================== 2. HISTORY TAB ====================
    ...
    with st.container(border=True):
        st.markdown("""<div style="border-top: 5px solid #FF6B00; margin: -16px -16px 20px -16px;"></div>""", unsafe_allow_html=True)
        conn = get_connection()
        try:
